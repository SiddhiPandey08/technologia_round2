...
// Reads your existing CSS custom properties so the shader auto-matches
// your gold/violet palette wherever it's rendered. Falls back to sensible
// defaults if a var isn't found.
...
export const GridScan = ({
  sensitivity = 0.45,
  lineThickness = 1,
  linesColor, // defaults to --border-strong
  scanColor, // defaults to --gold
  scanOpacity = 0.35,
  gridScale = 0.12,
  lineStyle = "solid",
  lineJitter = 0.06,
  ...
  const skewScale = THREE.MathUtils.lerp(0.05, 0.16, s);
  const smoothTime = THREE.MathUtils.lerp(0.5, 0.15, s);

  // gentle passive mouse-parallax (no face tracking)
  useEffect(() => {
    ...
    const onLeave = () => {
      leaveTimer = window.setTimeout(() => lookTarget.current.set(0, 0), 250);
    };
  ...
  useEffect(() => {
    ...
    const resolvedLines = srgbColor(
      linesColor || readCssVar("--border-strong", "#3a2f52"),
    );
    const resolvedScan = srgbColor(
      scanColor || readCssVar("--gold", "#f2b93b"),
    );
    ...
    const uniforms = {
      ...
      uLinesColor: { value: resolvedLines },
      uScanColor: { value: resolvedScan },
